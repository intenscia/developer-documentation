...
- Quota Logic should check three aspects of the [Show Quotas](#get-show-quotas) call when deciding to send a respondent to a survey:
  - Is the `SurveyStillLive` property `true`?
  - Is the `NumberOfRespondents` property greater than zero for __all__ SurveyQuotaIds that a respondent falls into? A respondent may fall into more than one quota.  For example, there may be quotas for gender and quotas for age, in which case a respondent may fall into two quotas, both of which must be open, for the respondent to pass.
  - Is the `NumberOfRespondents` property greater than zero for the `SurveyQuotaId` with a `SurveyQuotaType` value of `Total`? (Essentially, if that total quota is closed any respondent will result in an overquota, regardless of what other quotas may still be open. )
